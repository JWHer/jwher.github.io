...
쿠버네티스에 대해 이해가 부족하시다면 [이 글](/categories/infrastructure/kubernetes/welcome-to-kubernetes/welcome-to-kubernetes/) 을 먼저 읽는 걸 추천합니다!
...
~~[도커 설치](/categories/infrastructure/containerization/install-docker/install-docker)~~  
...
*kubectl이 없다면 [이 글](/categories/infrastructure/kubernetes/kubernetes-usage/kubernetes-usage/) 을 읽어보세요.* 😊
```bash
> kubectl version
```
